...
var app = app || {};
(function($){
	app.materialcreate = Element.View.extend({		
		componentcount : 0,
		customs : "",
		render : function(){
			this.getcomponentcustom();
			var self = this;			
			app.TemplateManager.get("../../modules/mto/tpl/tpl_materialcreate.html", function(template){
				...
				if(self.model.get("MaterialId")){
					...
					self.model.fetch({
							...
							if(self.options.componentcount > 0){
								$(self.el).find("#btnSave").text("This Material is used "+self.options.componentcount+" Components");
								$(self.el).find("#btnSave").attr("disabled", true);
								$(self.el).find("#btnSave").addClass("disable").removeClass("btn-primary").addClass("btn-danger");
							}
                            $(self.el).find("h2").html("Edit Material");
							self.renderscreen();
							self.setevent();
		...
		resetDic : function(){
			...
			$(self.el).find(".divDiscipline select").addClass("required");
			$(self.el).find(".divDiscipline select").attr("autofocus","true").attr("value","");
		},
		resetCat : function(){
			var self = this;	
			var cboCat = "";
			...
			$(self.el).find(".divTaskCategory select").addClass("required");
		},
		...
		resetType : function(){
			...
			$(self.el).find(".divTaskType select").addClass("required");
		},
		...
		resetCostCode : function(){
			var self = this;	
			
			var cboCostCode = new app.component.Combo({
				url : new app.costcodecombos().url,
				inline : true,
				selname : "Select CostCode",
				selected : self.model.get("CostCodeId")
			}).render().el;
			

			$(self.el).find(".divCostCode").html(cboCostCode);
		...
		validationcheck : function(){
			var self = this;
			var ret = true;
			var manhour = $(self.el).find("#txtManHour").val();
					
			if(Number(manhour) <= 0){				
				$(self.el).find("#txtManHour").parent().parent().removeClass("has-error").addClass("has-error");
				ret = false;
			}
			...
			$(this.el).find("#btnSave").on("click",function(e){
				...
				if(!self.validationcheck())
					...
					self.model.set({
						...
						MaterialId : self.model.get("MaterialId"),
						...
						CostCodeId : ($(self.el).find(".divCostCode option:selected").val()=="")?0:Number($(self.el).find(".divCostCode option:selected").val()),
		...
		getcomponentcustom : function(){
			...
			app.component.Call(
					app.config.domain + "/GlobalSettings/SigmaGlobalSettings.svc/rest/ComponentCustomFields",  
					"GET",                                      
					"",                                         
					function(data){                             
						self.customs = JSON.parse(data.JsonDataSet);
					}, 
					null                                        
				);
